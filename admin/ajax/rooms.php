...
if (isset($_POST['add_room'])) {
    ...
    // Decode and filter features and facilities
    ...
    // if (!$room_id) {
    //     echo "Failed to retrieve room ID.";
    //     exit();
    // }
    ...
    if ($stmt = mysqli_prepare($con, $q2)) {
        foreach ($facilities as $f) {
            ...
            // {
            //     $flag = 0;
            //     echo "Failed to insert facility ID $f for room ID $room_id.";
            //     break;
            // }
        }
        ...
        // echo "Query cannot be prepared for room facilities - insert.";
        // exit();
    }
    ...
    if ($stmt = mysqli_prepare($con, $q3)) {
        foreach ($features as $f) {
            ...
            //  {
            //     $flag = 0;
            //     echo "Failed to insert feature ID $f for room ID $room_id.";
            //     break;
            // }
        }
        mysqli_stmt_close($stmt);
        ...
        $flag = 0;
        die('Query cannot be prepared - insert');
        // echo "Query cannot be prepared for room features - insert.";
        // exit();
    }
...
if (isset($_POST['get_all_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if ($row['status'] == 1) {
            $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sn shadow-none'>active</button>
            ";
        } else {
            $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sn shadow-none'>inactive</button>
            ";
        }
        ...
        <tr class='align-middle'>
            ...
            <td>$status</td>
            <td>
                <button type='button' onclick='edit_details($row[id])' class='btn btn-primary' data-bs-toggle='modal'
...
if (isset($_POST['get_room_images'])) {
    ...
    while($row = mysqli_fetch_array($res))
    {
        if($row['thumb']==1){
            ...
            $thumb_btn = "<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary btn-sn shadow-none'>
                        <i class='bi bi-trash'></i>
                    </button>";
        }
        echo<<<data
