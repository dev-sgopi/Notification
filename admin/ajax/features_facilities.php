...
if (isset($_POST['rem_feature'])) {
    $frm_data = filteration($_POST);
    $values = [$frm_data['rem_feature']]; // Removed the space after 'rem_member'
...
if (isset($_POST['add_facility'])) {
    echo "Step 1: Facility POST received";
    $frm_data = filteration($_POST);
...
if (isset($_POST['rem_facility'])) {
    $frm_data = filteration($_POST);
    $values = [$frm_data['rem_facility']]; // Removed the space after 'rem_member'
    ...
    if(mysqli_num_rows($check_q)==0){
        ...
        if ($img && deleteImage($img['icon'], FACILITIES_FOLDER)) { // Added a check for $img to avoid errors if nothing is found
...
//     echo "Data received"; // For debugging
//     // Add logic to insert data into the database here
// } else {
